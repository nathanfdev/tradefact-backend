...
namespace Tradefact.Application.PurchaseOrders.Queries
{
    ...
    public class GetPurchaseOrderItemByIdQuery : IRequest<PurchaseOrderItemResource>
    {
        public Guid OrganisationId { get; set; }
        public Guid PurchaseOrderId { get; set; }
        public Guid PurchaseOrderItemId { get; set; }

        public class GetPurchaseOrderItemByIdQueryHandler : IRequestHandler<GetPurchaseOrderItemByIdQuery, PurchaseOrderItemResource>
        {
            private readonly IDbConnection _connection;
            private readonly ICurrentUserService _userService;

            public GetPurchaseOrderItemByIdQueryHandler(IDbConnection connection, ICurrentUserService userService)
            {
                _connection = connection;
                _userService = userService ?? throw new ArgumentNullException(nameof(userService));
            }

            public async Task<PurchaseOrderItemResource> Handle(GetPurchaseOrderItemByIdQuery request, System.Threading.CancellationToken cancellationToken)
            {
...
WHERE PI.PurchaseOrderId = @PurchaseOrderId AND PI.Id = @PurchaseOrderItemId ;
                ";

                using (IDbConnection conn = _connection)
                {
                    conn.Open();
                    PurchaseOrderItemDTO po = await conn.QueryFirstAsync<PurchaseOrderItemDTO>(sQuery, new
                    {
                    ...
                    try
                    {
                        return po.Adapt<PurchaseOrderItemResource>();
                    }
                    catch (Exception ex)
                    {
                        throw ex;
