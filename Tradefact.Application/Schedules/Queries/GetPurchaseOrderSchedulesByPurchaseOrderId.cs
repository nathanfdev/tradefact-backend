...
{
    public class GetPurchaseOrderSchedulesByPurchaseOrderIdQuery : IRequest<IPagedList<PurchaseOrderScheduleLineResource>>
    {
        public Guid OrganisationId { get; set; }
        public Guid PurchaseOrderId { get; set; }
        public PagedResultParameters Paging { get; set; }

        public class GetPurchaseOrderSchedulesByPurchaseOrderIdQueryHandler : IRequestHandler<GetPurchaseOrderSchedulesByPurchaseOrderIdQuery, IPagedList<PurchaseOrderScheduleLineResource>>
        {
            private readonly IDbConnection _connection;
            private readonly ILogger<GetPurchaseOrderSchedulesByPurchaseOrderIdQueryHandler> _logger;

            // Using DI to inject infrastructure persistence Repositories
            public GetPurchaseOrderSchedulesByPurchaseOrderIdQueryHandler(IDbConnection connection, ILogger<GetPurchaseOrderSchedulesByPurchaseOrderIdQueryHandler> logger)
            {
            ...
            public async Task<IPagedList<PurchaseOrderScheduleLineResource>> Handle(GetPurchaseOrderSchedulesByPurchaseOrderIdQuery request, System.Threading.CancellationToken cancellationToken)
            {
                string sQuery = @";WITH cte_schedule_lines AS (
                                        ...
                                        WHERE PO.Id = @purchaseOrderId AND PIS.Active = 1
