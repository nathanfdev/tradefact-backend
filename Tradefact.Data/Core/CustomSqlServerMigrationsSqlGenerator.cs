    ...
    public class CustomSqlServerMigrationsSqlGenerator : SqlServerMigrationsSqlGenerator
    {
       ...
       : base(dependencies, migrationsAnnotations)
        ...
        internal class ColumnOrderComparision : IComparer<AddColumnOperation>
        {
            ...
            {
                var orderX = Convert.ToInt32(x.FindAnnotation(CustomAnnotationNames.ColumnOrder)?.Value ?? 100);
                var orderY = Convert.ToInt32(y.FindAnnotation(CustomAnnotationNames.ColumnOrder)?.Value ?? 100);
