...
using Tradefact.Api.Common.Enumerators;
using Tradefact.Api.Common.Extensions;
using Tradefact.Api.Common.ViewModels;
...
namespace Tradefact.Api.Common.Validators
{
    public abstract class BlackslopeValidatorCollection<T> : AbstractValidator<T>
    {
        ...
        {
            var validationResult = await base.ValidateAsync(instance, cancellation).ConfigureAwait(false);
            HandleValidationFailure(validationResult, instance);
        }
