...
{
    public static class DailyForwarderBriefing
    {
        [FunctionName("DailyForwarderBriefing")]
        ...
        //public static void Run([TimerTrigger("*/5 * * * * *")] TimerInfo myTimer, ILogger log)
        {
            ...
            DailyBriefingForwarder variables = new DailyBriefingForwarder();
            ...
            using (SqlConnection conn = new SqlConnection(str))
            {
                ...
                var text = $@"declare @currentdate datetime = GETDATE();
                            ...
                            , cte_quotes AS (
                                select F.OrganisationId, F.Email, F.GivenName, P.Id [PartnershipId], SUM(CASE WHEN Q.[State] = 0 AND FM.GoodsReady > GETDATE() THEN 1 ELSE 0 END) [OutstandingQuotes]
                                ...
                                GROUP BY F.OrganisationId, P.Id, F.Email, F.GivenName
                            )
                                select 
                ...
                using (SqlCommand cmd = new SqlCommand(text, conn))
                {
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            ...
                            //if (variables.Email == "patrick.mccloy@importwise.co") variables.Email = "mikebucher@mail.com";
                            EmailSender.SendSimpleMessage(variables, JsonConvert.SerializeObject(variables), "dailybriefing", "Your Daily Tradefact Update");
                        }
