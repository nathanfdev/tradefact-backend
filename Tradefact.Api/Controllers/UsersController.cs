...
namespace Tradefact.Api.Controllers
{
    public class UsersController : BaseApiController
    {
        ...
        public UsersController(IOptions<AppSettings> appSettings, ILogger<UserController> logger, IBillingService billingService, TradefactDbContext context) : base(context)
        {
        ...
        public virtual async Task<IActionResult> Update([FromBody] UpdateUserRequest request)
        {
            ...
            existing_user.IsExposedToOtherOrganization = request.IsExposedToOtherOrganization;
            ...
            {
                var org = await _context.Organisations.FirstOrDefaultAsync(q => q.Id == this.OrganisationId);
                if (org != null && org.ChargebeeSubscriptionId != null)
                {
                    int numAddUsers = await _context.Users.CountAsync(q => q.OrganisationId == org.Id && q.Status != UserStatus.Disabled) - 1;
                    string subscriptionComment = $"Changed {existing_user.Email} status to {existing_user.Status.ToString()}";
